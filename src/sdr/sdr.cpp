...
static bool noOpen()
{
    gg::eprint("Net-only mode, no SDR device or file open.\n");
    return true;
}
...
{
    gg::eprint("Support for this SDR type was not enabled in this build.\n");
    return false;
}
...
void sdrShowHelp()
{
    gg::print("--device-type <type>     Select SDR type (default: %s)\n", sdr_handlers[0].name);
    gg::print("\n");
...
bool sdrHandleOption(int argc, char **argv, int *jptr)
{
    ...
    if (!strcmp(argv[j], "--device-type")) {
        if ((j+1) < argc) {
            ...
            gg::eprint("SDR type '%s' not recognized. ", argv[j]);
        }

        gg::eprint("Supported SDR types:\n");
        for (int32_t i = 0; sdr_handlers[i].name; ++i) {
            gg::eprint("  %s\n", sdr_handlers[i].name);
        }
