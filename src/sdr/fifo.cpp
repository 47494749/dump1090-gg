...
{
    if (!(overlap_buffer = static_cast<decltype(overlap_buffer)>(calloc(overlap, sizeof(overlap_buffer[0])))))
        goto nomem;
...
struct mag_buf *fifo_acquire(uint32_t timeout_ms)
{
    ...
    while (!fifo_halted && !fifo_freelist) {
        ...
        if (err) {
            if (err != ETIMEDOUT) {
                gg::eprint("fifo_acquire: pthread_cond_timedwait unexpectedly returned %s\n", strerror(err));
            }
...
struct mag_buf *fifo_dequeue(uint32_t timeout_ms)
{
    ...
    while (!fifo_head && !fifo_halted) {
        ...
        if (err) {
            if (err != ETIMEDOUT) {
                gg::eprint("fifo_dequeue: pthread_cond_timedwait unexpectedly returned %s\n", strerror(err));
            }
