...
bool hackRFHandleOption(int argc, char **argv, int *jptr)
{
    ...
    if (!strcmp(argv[j], "--lna-gain") && more) {
        ...
        if (HackRF.lna_gain % 8 != 0) {
            gg::eprint("Error: --lna-gain must be multiple of 8\n");
            return false;
        }

        if (HackRF.lna_gain > 40 || HackRF.lna_gain < 0) {
            gg::eprint("Error: --lna-gain range is 0 - 42\n");
            return false;
        }
        ...
        if (HackRF.vga_gain % 2 != 0) {
            gg::eprint("Error: --vga-gain must be multiple of 2\n");
            return false;
        }

        if (HackRF.vga_gain > 62 || HackRF.vga_gain < 0) {
            gg::eprint("Error: --vga-gain range is 0 - 62\n");
            return false;
        }
...
void hackRFShowHelp()
{
    gg::print("      HackRF-specific options (use with --device-type hackrf)\n");
    gg::print("\n");
    gg::print("--enable-amp             enable amplifier (disabled unless specified)\n");
    gg::print("--enable-antenna-power   enable DC power to the antenna connector (disabled unless specified)\n");
    gg::print("--lna-gain               set LNA gain (default: 32dB. Range 0-40 in 8dB steps)\n");
    gg::print("--vga-gain               set VGA gain (default: 50dB. Range 0-62 in 2dB steps)\n");
    gg::print("--samplerate             set sample rate (default: 2400000Hz)\n");
    gg::print("--ppm                    ppm correction (default: 0)\n");
    gg::print("\n");
}

static void show_config()
{
    gg::eprint("freq : %" PRIu64 "\n", HackRF.freq);
    gg::eprint("lna_gain : %d\n", HackRF.lna_gain);
    gg::eprint("vga_gain : %d\n", HackRF.vga_gain);
    gg::eprint("samplerate : %d\n", HackRF.rate);
    gg::eprint("ppm : %d\n", HackRF.ppm);
}
...
bool hackRFOpen()
{
    ...
    status = hackrf_init();
    if (status != 0) {
        gg::eprint("HackRF: hackrf_init failed with code %d\n", status);
        return false;
    }

    status = hackrf_open(&HackRF.device);
    if (status != 0) {
        gg::eprint("HackRF: hackrf_open failed with code %d\n", status);
        hackrf_exit();
        return false;
    }
    ...
    if (status != 0) {
        gg::eprint("HackRF: hackrf_set_freq failed with code %d\n", status);
    ...
    if (status != 0) {
        gg::eprint("HackRF: hackrf_set_sample_rate failed with code %d\n", status);
        hackrf_close(HackRF.device);
        hackrf_exit();
        return false;
    }
    ...
    if (status != 0) {
        gg::eprint("HackRF: hackrf_set_amp_enable failed with code %d\n", status);
        hackrf_close(HackRF.device);
        hackrf_exit();
        return false;
    }
    ...
    if (status != 0) {
        gg::eprint("HackRF: hackrf_set_lna_gain failed with code %d\n", status);
        hackrf_close(HackRF.device);
        hackrf_exit();
        return false;
    }
    ...
    if (status != 0) {
        gg::eprint("HackRF: hackrf_set_vga_gain failed with code %d\n", status);
        hackrf_close(HackRF.device);
        hackrf_exit();
        return false;
    }
    ...
    if (status != 0) {
        gg::eprint("HackRF: hackrf_set_antenna_enable failed with code %d\n", status);
        hackrf_close(HackRF.device);
        hackrf_exit();
        return false;
    }
    ...
    if (!HackRF.converter) {
        gg::eprint("HackRF: can't initialize sample converter\n");
        return false;
    }
...
{
    if (!HackRF.device) {
        gg::eprint("hackRFRun: HackRF.device = NULL\n");
        return;
    }
    ...
    if (status != 0) { 
        gg::eprint("hackrf_start_rx failed\n");
        hackrf_close(HackRF.device); 
        hackrf_exit(); 
        exit (1); 
    }
    ...
    gg::eprint("HackRF stopped streaming %d\n", hackrf_is_streaming(HackRF.device));
}
