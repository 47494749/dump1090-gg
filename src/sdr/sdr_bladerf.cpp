...
{
    gg::print("      bladeRF-specific options (use with --device-type bladerf)\n");
    gg::print("\n");
    gg::print("--device <ident>         select device by bladeRF 'device identifier'\n");
    gg::print("--bladerf-fpga <path>    use alternative FPGA bitstream ('' to disable FPGA load)\n");
    gg::print("--bladerf-decimation <N> assume FPGA decimates by a factor of N\n");
    gg::print("--bladerf-bandwidth <hz> set LPF bandwidth ('bypass' to bypass the LPF)\n");
    gg::print("\n");
}
...
static void show_config()
{
    ...
    if ((status = bladerf_get_sample_rate(BladeRF.device, BLADERF_MODULE_RX, &rate)) < 0 ||
        ...
        gg::eprint("bladeRF: couldn't read back device configuration\n");
        return;
    }

    gg::eprint("bladeRF: sampling rate: %.1f MHz\n", rate/1e6);
    gg::eprint("bladeRF: frequency:     %.1f MHz\n", freq/1e6);
    gg::eprint("bladeRF: LNA gain:      %ddB\n", lna_gain_db(lna_gain));
    gg::eprint("bladeRF: RXVGA1 gain:   %ddB\n", rxvga1_gain);
    gg::eprint("bladeRF: RXVGA2 gain:   %ddB\n", rxvga2_gain);

    switch (lpf_mode) {
    case BLADERF_LPF_NORMAL:
        gg::eprint("bladeRF: LPF bandwidth: %.2f MHz\n", lpf_bw/1e6);
        break;
    case BLADERF_LPF_BYPASSED:
        gg::eprint("bladeRF: LPF bypassed\n");
        break;
    case BLADERF_LPF_DISABLED:
        gg::eprint("bladeRF: LPF disabled\n");
        break;
    default:
        gg::eprint("bladeRF: LPF in unknown state\n");
        break;
    }

    gg::eprint("bladeRF: calibration settings:\n");
    gg::eprint("  LMS DC adjust:     I=%d Q=%d\n", lms_dc_i, lms_dc_q);
    gg::eprint("  FPGA phase adjust: %+.3f degrees\n", fpga_phase * 10.0 / 4096);
    gg::eprint("  FPGA gain adjust:  %+.3f\n", fpga_gain * 1.0 / 4096);
    gg::eprint("  LMS LPF tuning:    %d\n", dc_cals.lpf_tuning);
    gg::eprint("  LMS RX LPF filter: I=%d Q=%d\n", dc_cals.rx_lpf_i, dc_cals.rx_lpf_q);
    gg::eprint("  LMS RXVGA2 DC ref: %d\n", dc_cals.dc_ref);
    gg::eprint("  LMS RXVGA2A:       I=%d Q=%d\n", dc_cals.rxvga2a_i, dc_cals.rxvga2a_q);
    gg::eprint("  LMS RXVGA2B:       I=%d Q=%d\n", dc_cals.rxvga2b_i, dc_cals.rxvga2b_q);
...
bool bladeRFOpen()
{
    ...
    if ((status = bladerf_open(&BladeRF.device, Modes.dev_name)) < 0) {
        gg::eprint("Failed to open bladeRF: %s\n", bladerf_strerror(status));
        goto error;
    }
    ...
    if (BladeRF.fpga_path) {
        ...
        if ((status = bladerf_get_fpga_size(BladeRF.device, &size)) < 0) {
            gg::eprint("bladerf_get_fpga_size failed: %s\n", bladerf_strerror(status));
            goto error;
        }
        ...
        case BLADERF_FPGA_115KLE:
            ...
            break;
        default:
            gg::eprint("bladeRF: unknown FPGA size, skipping FPGA load");
            fpga_path = NULL;
            break;
        }
    }
    ...
    if (fpga_path && fpga_path[0]) {
        gg::eprint("bladeRF: loading FPGA bitstream from %s\n", fpga_path);
        if ((status = bladerf_load_fpga(BladeRF.device, fpga_path)) < 0) {
            gg::eprint("bladerf_load_fpga() failed: %s\n", bladerf_strerror(status));
            goto error;
        }
    }
    ...
    if (!BladeRF.converter) {
        gg::eprint("can't initialize sample converter\n");
        goto error;
    }
                                    ...
                                    void *user_data)
{
    ...
    for (uint32_t offset = 0; offset < num_samples * 4; offset += BladeRF.block_size) {
        ...
        if (metadata_magic != 0x12344321) {
            ...
            if (!first_buffer) {
                gg::eprint("bladeRF: wrong metadata header magic value, skipping rest of buffer\n");
            }
            break;
        }
...
void bladeRFRun()
{
    ...
    if ((status = bladerf_init_stream(&stream,
                                      ...
                                      /* user_data */ NULL)) < 0) {
        gg::eprint("bladerf_init_stream() failed: %s\n", bladerf_strerror(status));
        goto out;
    }

    {
    uint32_t ms_per_transfer = 1000 * MODES_MAG_BUF_SAMPLES / Modes.sample_rate;
    if ((status = bladerf_set_stream_timeout(BladeRF.device, static_cast<bladerf_direction>(BLADERF_MODULE_RX), ms_per_transfer * (transfers + 2))) < 0) {
        gg::eprint("bladerf_set_stream_timeout() failed: %s\n", bladerf_strerror(status));
        goto out;
    }

    if ((status = bladerf_enable_module(BladeRF.device, BLADERF_MODULE_RX, true) < 0)) {
        gg::eprint("bladerf_enable_module(RX, true) failed: %s\n", bladerf_strerror(status));
        goto out;
    }

    timeouts = 0; // reset to zero when we get a callback with some data
 retry:
    if ((status = bladerf_stream(stream, static_cast<bladerf_channel_layout>(BLADERF_MODULE_RX))) < 0) {
        gg::eprint("bladerf_stream() failed: %s\n", bladerf_strerror(status));
        if (status == BLADERF_ERR_TIMEOUT) {
            if (++timeouts < 5)
                goto retry;
            gg::eprint("bladerf is wedged, giving up.\n");
        }
        goto out;
    }
    } // end of block for ms_per_transfer

 out:
    if ((status = bladerf_enable_module(BladeRF.device, BLADERF_MODULE_RX, false) < 0)) {
        gg::eprint("bladerf_enable_module(RX, false) failed: %s\n", bladerf_strerror(status));
    }
