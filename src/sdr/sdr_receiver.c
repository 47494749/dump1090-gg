...
bool rxOpen(sdr_receiver_t *rx)
{
    ...
    if (!ops) {
        fprintf(stderr, "rx[%d]: no SDR backend available\n", rx->id);
        rx->state = RX_STATE_ERROR;
        return false;
    }
    rx->backend_ops = ops;
    ...
    if (rx->config.serial[0]) {
        ...
        if (dev_index < 0) {
            fprintf(stderr, "rx[%d]: no free SDR device available\n", rx->id);
            rx->state = RX_STATE_ERROR;
            return false;
        }
    }
    ...
    if (!sdev) {
        fprintf(stderr, "rx[%d]: error opening device #%d via %s\n", rx->id, dev_index, ops->name);
        rx->state = RX_STATE_ERROR;
        return false;
    }
    rx->backend_dev = sdev;
...
static void *rx_reader_thread(void *arg)
{
    ...
    if (!Modes.exit && rx->state != RX_STATE_STOPPING && rx->usb_error_count > 0) {
        ...
        if (!new_dev) {
            fprintf(stderr, "rx[%d]: failed to reopen device, giving up\n", rx->id);
            rx->state = RX_STATE_ERROR;
            goto done;
        }
    ...
    if (!Modes.exit && rx->state != RX_STATE_STOPPING) {
        fprintf(stderr, "rx[%d]: read_async returned unexpectedly, device may be lost\n", rx->id);
        rx->state = RX_STATE_ERROR;
    }
                   ...
                   int32_t new_ppm, uint32_t new_freq, double new_sample_rate)
{
    ...
    if (rx->state == RX_STATE_ERROR && rx->thread_started) {
        ...
        rx->thread_started = false;
        rx->state = RX_STATE_OPEN;
        fprintf(stderr, "rx[%d]: rxReconfigure: recovered from error state\n", rx->id);
    }
