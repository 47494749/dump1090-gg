...
void soapyShowHelp()
{
    gg::print("      SoapySDR-specific options (use with --device-type soapy)\n");
    gg::print("\n");
    gg::print("--device <string>          select/configure device\n");
    gg::print("--channel <num>            select channel if device supports multiple channels (default: 0)\n");
    gg::print("--antenna <string>         select antenna (default depends on device)\n");
    gg::print("--bandwidth <hz>           set the baseband filter width (default: 3MHz, SDRPlay: 5MHz)\n");
    gg::print("--enable-agc               enable Automatic Gain Control if supported by device\n");
    gg::print("--gain-element <name>:<db> set gain in dB for a named gain element\n");
    gg::print("\n");
}
...
bool soapyHandleOption(int argc, char **argv, int *jptr)
{
    ...
    if (!strcmp(argv[j], "--channel")) {
        ...
        if (! (SOAPY.gain_elements = static_cast<decltype(SOAPY.gain_elements)>(realloc(SOAPY.gain_elements, SOAPY.num_gain_elements * sizeof(*SOAPY.gain_elements)))) ) {
            perror("realloc");
            abort();
        }
...
static void soapyShowDevices(SoapySDRKwargs *results, size_t length)
{
    for (size_t i = 0; i < length; ++i) {
        ...
        for (size_t j = 0; j < results[i].size; ++j) {
            if (j) gg::eprint(", ");
            gg::eprint("%s=%s", results[i].keys[j], results[i].vals[j]);
        }
        gg::eprint("\n");
    }
}
...
bool soapyOpen(void)
{
    ...
    if (length == 0) {
        SoapySDRKwargsList_clear(results, length);
        gg::eprint("soapy: no matching devices found; available devices:\n");
        soapyShowAllDevices();
        return false;
    }
    if (length > 1) {
        gg::eprint("soapy: more than one matching device found; matching devices:\n");
        soapyShowDevices(results, length);
        SoapySDRKwargsList_clear(results, length);
        gg::eprint("soapy: please select a single device with --device\n");
        return false;
    }

    gg::eprint("soapy: selected device:  ");
    for (size_t j = 0; j < results[0].size; j++) {
        if (j) gg::eprint(", ");
        gg::eprint("%s=%s", results[0].keys[j], results[0].vals[j]);
    }
    gg::eprint("\n");
    SoapySDRKwargsList_clear(results, length);
    ...
    if (!SOAPY.dev) {
        gg::eprint("soapy: failed to create device: %s\n", SoapySDRDevice_lastError());
        return false;
    }

    SoapySDRKwargs result = SoapySDRDevice_getHardwareInfo(SOAPY.dev);
    if (result.size > 0) {
        gg::eprint("soapy: hardware info:  ");
        for (size_t i = 0; i < result.size; ++i) {
            if (i) gg::eprint(", ");
            gg::eprint("%s=%s", result.keys[i], result.vals[i]);
        }
        gg::eprint("\n");
    }
    ...
    if (hw_key) {
        gg::eprint("soapy: hardware key:    %s\n", hw_key);
    ...
    if (SoapySDRDevice_setFrequency(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, Modes.freq, NULL) != 0) {
        gg::eprint("soapy: setFrequency failed: %s\n", SoapySDRDevice_lastError());
        goto error;
    }
    ...
    if (SOAPY.enable_agc) {
        if (has_agc) {
            if (SoapySDRDevice_setGainMode(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, 1) != 0) {
                gg::eprint("soapy: setGainMode failed: %s\n", SoapySDRDevice_lastError());
                goto error;
            }
        }
        else {
            gg::eprint("soapy: device does not support enabling AGC\n");
            goto error;
        }
    } else {
        // Manual gain path
        ...
        if (SoapySDRDevice_setGain(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, gain) < 0) {
            gg::eprint("soapy: setGain(%.1fdB) failed\n", gain);
            goto error;
        }
        ...
        for (int32_t i = 0; i < SOAPY.num_gain_elements; ++i) {
            ...
            if (!sep || !sep[1]) {
                gg::eprint("soapy: don't understand a gain element setting of '%s' (should be formatted as <element>:<db>)\n", element);
                goto error;
            }
            ...
            *sep = 0;
            ...
            if ((gain == 0 && !endptr) || *endptr) {
                gg::eprint("soapy: don't understand a gain value of '%s' for gain element %s (should be a floating-point gain in dB)\n", sep + 1, element);
                goto error;
            }

            if (SoapySDRDevice_setGainElement(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, element, gain) != 0) {
                gg::eprint("soapy: setGainElement(%s,%.1fdB) failed: %s\n", element, gain, SoapySDRDevice_lastError());
                goto error;
            }
        }
    }
    ...
    if (SoapySDRDevice_setBandwidth(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, SOAPY.bandwidth) != 0) {
        gg::eprint("soapy: setBandwidth(%.1f MHz) failed: %s\n", SOAPY.bandwidth / 1e6, SoapySDRDevice_lastError());
        goto error;
    }
    ...
    if (SoapySDRDevice_hasDCOffset(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel)) {
        ...
        if (!SoapySDRDevice_getDCOffset(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, &offsetI, &offsetQ)) {
            gg::eprint("soapy: DC offset:  I=%.3f Q=%.3f\n", offsetI, offsetQ);
        }
    }
    ...
    if (SoapySDRDevice_hasIQBalance(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel)) {
        ...
        if (!SoapySDRDevice_getIQBalance(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, &balanceI, &balanceQ)) {
            gg::eprint("soapy: IQ balance is I=%.1f, Q=%.1f\n", balanceI, balanceQ);
        }
    }
    ...
    if ((SOAPY.stream = SoapySDRDevice_setupStream(SOAPY.dev, SOAPY_SDR_RX, SOAPY_SDR_CS16, channels, 1, &stream_args)) == NULL) {
        gg::eprint("soapy: setupStream failed: %s\n", SoapySDRDevice_lastError());
        goto error;
    }
    ...
    if (!SOAPY.converter) {
        gg::eprint("soapy: can't initialize sample converter\n");
        goto error;
    }
...
void soapyRun()
{
    ...
    if (SoapySDRDevice_activateStream(SOAPY.dev, SOAPY.stream, 0, 0, 0) != 0) {
        gg::eprint("soapy: activateStream failed: %s\n", SoapySDRDevice_lastError());
        return;
    }

    uint8_t* buf;
    const int32_t buffer_elements = MODES_MAG_BUF_SAMPLES; // 131072
    buf = static_cast<decltype(buf)>(malloc(buffer_elements * 4));
    ...
    while (!Modes.exit) {
        ...
        if (samples_read <= 0) {
            gg::eprint("soapy: readStream failed: %s\n", SoapySDRDevice_lastError());
            return;
        }

        struct mag_buf *outbuf = fifo_acquire(0 /* no wait */);
        if (!outbuf) {
            gg::eprint("soapy: fifo is full, dropping samples\n");
...
int32_t soapySetGain(int32_t step) {
    ...
    if (SoapySDRDevice_setGain(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, gainDb) != 0) {
        gg::eprint("soapy: setGain(%.1fdB) failed: %s\n", gainDb, SoapySDRDevice_lastError());
        return -1;
    }

    gg::eprint("soapy: total gain set to %.1fdB", SoapySDRDevice_getGain(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel));
    size_t length = 0;
    char** gain_elements = SoapySDRDevice_listGains(SOAPY.dev, SOAPY_SDR_RX, SOAPY.channel, &length);
    for (size_t i = 0; i < length; i++) {
        fprintf(stderr, "; %s=%.1fdB", gain_elements[i],
