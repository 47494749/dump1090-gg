...
typedef struct sdr_backend_ops {
    ...
    // Enumeration
    int  (*enumerate)(sdr_dev_info_t *devs, int max_devs);

    // Lifecycle
    sdr_device_t *(*open_by_index)(int index);
    sdr_device_t *(*open_by_serial)(const char *serial);
    void (*close)(sdr_device_t *dev);

    // Configuration
    int  (*set_frequency)(sdr_device_t *dev, uint32_t freq_hz);
    uint32_t (*get_frequency)(sdr_device_t *dev);
    int  (*set_sample_rate)(sdr_device_t *dev, uint32_t rate_hz);
    int  (*set_gain_mode)(sdr_device_t *dev, int manual);   // 0=auto, 1=manual
    int  (*set_gain)(sdr_device_t *dev, int gain_tenth_db);
    int  (*get_gain)(sdr_device_t *dev);                    // returns gain in tenth dB
    int  (*set_freq_correction)(sdr_device_t *dev, int ppm);
    int  (*set_agc)(sdr_device_t *dev, int enable);
    int  (*set_direct_sampling)(sdr_device_t *dev, int mode);
    int  (*reset_buffer)(sdr_device_t *dev);

    // Gain table
    int  (*get_tuner_gains)(sdr_device_t *dev, int *gains, int max_count);
    int  (*get_tuner_type)(sdr_device_t *dev);

    // Streaming
    int  (*read_async)(sdr_device_t *dev, sdr_async_cb_t cb, void *ctx,
                       uint32_t buf_count, uint32_t buf_size);
    int  (*cancel_async)(sdr_device_t *dev);
    int  (*read_sync)(sdr_device_t *dev, uint8_t *buf, uint32_t len, int *n_read);

    // Extended capabilities (may be NULL if not supported)
    int  (*get_tuner_caps)(sdr_device_t *dev, sdr_tuner_caps_t *caps);
    int  (*set_bandwidth)(sdr_device_t *dev, uint32_t bw_khz);
    int  (*set_lna_gain)(sdr_device_t *dev, int index);
    int  (*set_mixer_gain)(sdr_device_t *dev, int index);
    int  (*set_vga_gain)(sdr_device_t *dev, int index);
    int  (*read_tuner_reg)(sdr_device_t *dev, uint8_t reg, uint8_t *val);
    int  (*write_tuner_reg)(sdr_device_t *dev, uint8_t reg, uint8_t val);
} sdr_backend_ops_t;
...
struct sdr_device {
    ...
    int      current_gain;          // cached gain (tenth dB)
    sdr_tuner_type_t tuner_type;    // cached tuner type
    volatile int async_running;     // nonzero while read_async should stay blocked
};
...
static inline int sdr_set_frequency(sdr_device_t *dev, uint32_t freq_hz)
...
static inline int sdr_set_sample_rate(sdr_device_t *dev, uint32_t rate_hz)
{ return dev->ops->set_sample_rate(dev, rate_hz); }

static inline int sdr_set_gain_mode(sdr_device_t *dev, int manual)
{ return dev->ops->set_gain_mode(dev, manual); }

static inline int sdr_set_gain(sdr_device_t *dev, int gain_tenth_db)
{ return dev->ops->set_gain(dev, gain_tenth_db); }

static inline int sdr_get_gain(sdr_device_t *dev)
{ return dev->ops->get_gain(dev); }

static inline int sdr_set_freq_correction(sdr_device_t *dev, int ppm)
{ return dev->ops->set_freq_correction(dev, ppm); }

static inline int sdr_set_agc(sdr_device_t *dev, int enable)
{ return dev->ops->set_agc(dev, enable); }

static inline int sdr_reset_buffer(sdr_device_t *dev)
{ return dev->ops->reset_buffer(dev); }

static inline int sdr_get_tuner_gains(sdr_device_t *dev, int *gains, int max)
{ return dev->ops->get_tuner_gains(dev, gains, max); }

static inline int sdr_get_tuner_type(sdr_device_t *dev)
{ return dev->ops->get_tuner_type(dev); }

static inline int sdr_read_async(sdr_device_t *dev, sdr_async_cb_t cb, void *ctx,
                                 uint32_t buf_count, uint32_t buf_size)
{ return dev->ops->read_async(dev, cb, ctx, buf_count, buf_size); }

static inline int sdr_cancel_async(sdr_device_t *dev)
{ return dev->ops->cancel_async(dev); }

static inline int sdr_read_sync(sdr_device_t *dev, uint8_t *buf, uint32_t len, int *n_read)
{ return dev->ops->read_sync(dev, buf, len, n_read); }

// Extended ops — return -1 if not supported by backend
static inline int sdr_get_tuner_caps(sdr_device_t *dev, sdr_tuner_caps_t *caps)
{ return dev->ops->get_tuner_caps ? dev->ops->get_tuner_caps(dev, caps) : -1; }

static inline int sdr_set_bandwidth(sdr_device_t *dev, uint32_t bw_khz)
{ return dev->ops->set_bandwidth ? dev->ops->set_bandwidth(dev, bw_khz) : -1; }

static inline int sdr_set_lna_gain(sdr_device_t *dev, int index)
{ return dev->ops->set_lna_gain ? dev->ops->set_lna_gain(dev, index) : -1; }

static inline int sdr_set_mixer_gain(sdr_device_t *dev, int index)
{ return dev->ops->set_mixer_gain ? dev->ops->set_mixer_gain(dev, index) : -1; }

static inline int sdr_set_vga_gain(sdr_device_t *dev, int index)
{ return dev->ops->set_vga_gain ? dev->ops->set_vga_gain(dev, index) : -1; }

static inline int sdr_read_tuner_reg(sdr_device_t *dev, uint8_t reg, uint8_t *val)
