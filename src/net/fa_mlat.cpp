...
static int32_t udp_start(struct fa_mlat_udp *u, const char *host, int32_t port, uint32_t key) {
    ...
    if (u->fd < 0) {
        gg::eprint("FA-MLAT: UDP socket failed: %s\n", strerror(errno));
        freeaddrinfo(res);
        return -1;
    }
    ...
    if (connect(u->fd, res->ai_addr, res->ai_addrlen) < 0) {
        ...
        gg::eprint("FA-MLAT: UDP connect warning: %s\n", strerror(errno));
    }
...
static void *fa_mlat_thread_entry(void *arg) {
    ...
    std::string host = FaMlat.udp_host;
    int32_t port = FaMlat.udp_port;
    uint32_t key = FaMlat.udp_key;
    pthread_mutex_unlock(&FaMlat.ctl_mutex);

    // Start UDP transport
    if (udp_start(&udp, host.c_str(), port, key) < 0) {
        gg::eprint("FA-MLAT: failed to start UDP transport, thread exiting\n");
    ...
    gg::eprint("FA-MLAT: thread stopped\n");
    FaMlat.thread_running = 0;
    return NULL;
}
...
void faMlatEnable(const char *host, int32_t port, uint32_t key) {
    ...
    if (pthread_create(&FaMlat.thread, NULL, fa_mlat_thread_entry, NULL) != 0) {
        gg::eprint("FA-MLAT: failed to create thread: %s\n", strerror(errno));
        FaMlat.thread_running = 0;
        return;
    }

    gg::eprint("FA-MLAT: thread started (udp %s:%d key=%u)\n", host, port, key);
}
