...
static void opensky_save_serial(const char *path, int32_t serial) {
    FILE *f = fopen(path, "w");
    if (!f) {
        gg::eprint("OpenSky: could not save serial to %s: %s\n", path, strerror(errno));
        return;
    }
    gg::fprint(f, "[Device]\nserial = %d\n", serial);
    fclose(f);
...
static int32_t opensky_recv_serial(int32_t fd) {
    ...
    if (opensky_read_all(fd, hdr, 4, 60) != 0) {
        gg::eprint("OpenSky: timeout waiting for serial number\n");
        return 0;
    }
...
void openskyClientInit(void) {
    OpenSkyConfig = {};
    OpenSkyConfig.host = "collector.opensky-network.org";
    OpenSkyConfig.port = 10004;
    OpenSkyConfig.serial_file = "/var/lib/dump1090-gg/opensky-serial.conf";
    if (!opensky_queue)
    ...
    while (atomic_load(&feeders_running)) {
        ...
        if (!atomic_load(&net_available)) {
            ...
            if (fd >= 0) {
                ...
                fd = -1;
                logged_in = 0;
                gg::eprint("OpenSky: disconnected (internet offline)\n");
            }
            usleep(500000);
            continue;
        }
        ...
        if (fd < 0 && now >= next_reconnect) {
            fd = opensky_tcp_connect(OpenSkyConfig.host.c_str(), OpenSkyConfig.port);
            if (fd < 0) {
                fprintf(stderr, "OpenSky: failed to connect to %s:%d, retry in 30s\n",
                        OpenSkyConfig.host.c_str(), OpenSkyConfig.port);
            ...
            if (opensky_send_login(fd) != 0) {
                gg::eprint("OpenSky: login send failed\n");
                goto reconnect;
            }
            ...
            if (OpenSkyConfig.serial == 0) {
                ...
                OpenSkyConfig.serial = opensky_load_serial(OpenSkyConfig.serial_file.c_str());
            }

            if (OpenSkyConfig.serial != 0) {
                // Send known serial
                if (opensky_send_serial(fd, OpenSkyConfig.serial) != 0) {
                    gg::eprint("OpenSky: serial send failed\n");
                    goto reconnect;
                }
                gg::eprint("OpenSky: sent serial %d\n", OpenSkyConfig.serial);
            } else {
                // Request new serial
                if (opensky_send_serial_req(fd) != 0) {
                    gg::eprint("OpenSky: serial request failed\n");
                    goto reconnect;
                }
                gg::eprint("OpenSky: requesting new serial number\n");

                int32_t new_serial = opensky_recv_serial(fd);
                if (new_serial == 0) {
                    gg::eprint("OpenSky: failed to get serial number\n");
                    goto reconnect;
                }

                OpenSkyConfig.serial = new_serial;
                gg::eprint("OpenSky: got serial %d\n", new_serial);
                opensky_save_serial(OpenSkyConfig.serial_file.c_str(), new_serial);

                // Echo serial back to server
                if (opensky_send_serial(fd, OpenSkyConfig.serial) != 0) {
                    gg::eprint("OpenSky: serial echo failed\n");
                    goto reconnect;
                }
            }
            ...
            if (opensky_send_gps(fd, OpenSkyConfig.lat, OpenSkyConfig.lon, OpenSkyConfig.alt) != 0) {
                gg::eprint("OpenSky: GPS send failed\n");
                goto reconnect;
            }
            ...
            // Step 4: Username
            if (!OpenSkyConfig.username.empty()) {
                if (opensky_send_username(fd, OpenSkyConfig.username.c_str()) != 0) {
                    gg::eprint("OpenSky: username send failed\n");
                    goto reconnect;
                }
                gg::eprint("OpenSky: sent username '%s'\n", OpenSkyConfig.username.c_str());
            }
            ...
            fprintf(stderr, "OpenSky: login complete (serial=%d, user=%s)\n",
                    OpenSkyConfig.serial, OpenSkyConfig.username.c_str());
        }
        ...
        if (fd >= 0 && logged_in) {
            ...
            while (msg_queue_pop(opensky_queue, &mm)) {
                ...
                if (w < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
                    gg::eprint("OpenSky: write error: %s\n", strerror(errno));
                    goto reconnect;
                }
                sent++;
            }
            ...
            if (now - last_heartbeat >= 30000) {
                if (write(fd, heartbeat, sizeof(heartbeat)) < 0) {
                    if (errno != EAGAIN && errno != EWOULDBLOCK) {
                        gg::eprint("OpenSky: heartbeat error: %s\n", strerror(errno));
                        goto reconnect;
                    }
                }
                last_heartbeat = now;
            ...
            if (r == 0) {
                gg::eprint("OpenSky: connection closed by server\n");
                goto reconnect;
            } else if (r < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
                gg::eprint("OpenSky: read error: %s\n", strerror(errno));
                goto reconnect;
            }
