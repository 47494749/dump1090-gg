...
static int try_decode_message(uint32_t addr, const uint8_t *data, int len,
    ...
    printf("CPDLC %s %06X MID=%d", dir, addr, msg_id);
    if (msg_ref >= 0) printf(" REF=%d", msg_ref);
    if (has_timestamp)
    ...
    for (int i = 0; i < elem_count; i++) {
        ...
        if (elem_idx < 0) { printf("(decode error at element %d)", i); break; }
        ...
        if (elem_idx < table_size && table[elem_idx].text) {
            ...
            if (decode_param(&u, elem->ptype, elem->text, param_buf, sizeof(param_buf)) < 0)
                printf("%s%d %s (?)", pfx, elem_idx, elem->text);
            else
                printf("%s%d %s", pfx, elem_idx, param_buf);
        } else {
            printf("%s%d", pfx, elem_idx);
        }
    }
