...
static int32_t flagCollisions(struct errorinfo *table, int32_t tablesize, int32_t offset, int32_t startbit, int32_t endbit, uint32_t base_syndrome, int32_t error_bit, int32_t first_error, int32_t last_error)
{
    ...
    for (i = startbit; i < endbit; ++i) {
        ...
        if (error_bit >= first_error) {
            struct errorinfo *collision = static_cast<struct errorinfo*>(bsearch(&ei, table, tablesize, sizeof(struct errorinfo), syndrome_compare));
...
static struct errorinfo *prepareErrorTable(int32_t bits, int32_t max_correct, int32_t max_detect, int32_t *size_out)
{
...
#ifdef CRCDEBUG
    gg::eprint("Preparing syndrome table to correct up to %d-bit errors (detecting %d-bit errors) in a %d-bit message (max %d entries)\n", max_correct, max_detect, bits, maxsize);
#endif

    table = static_cast<decltype(table)>(malloc(maxsize * sizeof(struct errorinfo)));
...
#ifdef CRCDEBUG
    gg::eprint("%d syndromes (expected %d).\n", usedsize, maxsize);
    gg::eprint("Sorting syndromes..\n");
#endif
...
#ifdef CRCDEBUG
    {
        ...
        for (i = 1; i <= max_correct; ++i) {
            ...
            gg::eprint("  %d undetectable %d-bit errors\n", count, i);
        }
    }
#endif
    ...
    if (j < usedsize) {
#ifdef CRCDEBUG
        gg::eprint("Discarded %d collisions.\n", usedsize - j);
#endif
        usedsize = j;
    }
...
#ifdef CRCDEBUG
            gg::eprint("Discarded %d flagged collisions.\n", usedsize - j);
#endif
            usedsize = j;
        }
    }
    ...
    if (usedsize < maxsize) {
#ifdef CRCDEBUG
        gg::eprint("Shrinking table from %d to %d..\n", maxsize, usedsize);
        table = static_cast<decltype(table)>(realloc(table, usedsize * sizeof(struct errorinfo)));
#endif
    }
    ...
    *size_out = usedsize;
    ...
    {
        // Check the table.
        uint8_t *msg = static_cast<uint8_t*>(malloc(bits/8));
        ...
        for (i = 0; i < usedsize; ++i) {
            ...
            if (result != ei->syndrome) {
                gg::eprint("PROBLEM: entry %6d/%6d  syndrome %06x  errors %d  bits ", i, usedsize, ei->syndrome, ei->errors);
                for (j = 0; j < ei->errors; ++j)
                    gg::eprint("%3d ", ei->bit[j]);
                gg::eprint(" checksum %06x\n", result);
            }
        }
        free(msg);

        // Show the table stats
        gg::eprint("Syndrome table summary:\n");
        for (i = 1; i <= max_correct; ++i) {
            ...
            gg::eprint("  %d entries for %d-bit errors (%d possible, %d%% coverage)\n", count, i, possible, 100 * count / possible);
        }

        gg::eprint("  %d entries total\n", usedsize);
    }
#endif
...
void modesChecksumInit(int32_t fixBits)
{
    ...
    default:
        ...
        gg::eprint("Preparing error correction tables.. ");
...
struct errorinfo *modesChecksumDiagnose(uint32_t syndrome, int32_t bitlen)
{
    ...
    return static_cast<struct errorinfo*>(bsearch(&ei, table, tablesize, sizeof(struct errorinfo), syndrome_compare));
}
...
int main(int argc, char **argv)
{
    ...
    if (argc < 3) {
        gg::eprint("syntax: crctests <ncorrect> <ndetect>\n");
        return 1;
    }
  ...
  syndrome 2 = 00606C  bits=[ 28 43 ]
    */
    ...
    gg::eprint("checking %d syndromes for DF11 collisions..\n", shortlen);
            ...
            for (j = i+1; j < shortlen; ++j) {
                if ((shorttable[i].syndrome & 0xFFFF80) == (shorttable[j].syndrome & 0xFFFF80)) {
                    ...
                    if (mismatch) {
                        ...
                        for (k = 0; k < shorttable[i].errors; ++k)
                            gg::eprint(" %d", shorttable[i].bit[k]);
                        gg::eprint(" ]\n");
                        ...
                        for (k = 0; k < shorttable[j].errors; ++k)
                            gg::eprint(" %d", shorttable[j].bit[k]);
                        gg::eprint(" ]\n");
                    }
                } else {
                    break;
