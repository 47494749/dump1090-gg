...
void demodulate2400(struct mag_buf *mag)
{
    ...
    for (j = last_message_end; j < mlen; j++) {
        ...
        if (preamble[1] > preamble[2] &&                                       // 1
            ...
            // CRC rescue mode: preamble pattern not recognized, but we have
            // a rising edge. Try brute-force bit extraction + CRC check.
            // Skip SNR and quiet-bits checks; rely solely on CRC validity.
            crc_rescue_attempt = 1;
            high = (preamble[1] + preamble[12]) / 2;
            base_signal = preamble[1] + preamble[12];
            base_noise = preamble[5] + preamble[6];
            goto crc_rescue_try_phases;
        }
    ...
    crc_rescue_try_phases:
        ...
        if (decodeModesMessage(&mm, bestmsg) < 0) {
            ...
            if (crc_rescue_attempt) {
                Modes.stats_current.demod_crc_rescued++;
            }
        }
...
static void draw_modeac(uint16_t *m, uint32_t modeac, uint32_t f1_clock, uint32_t noise_threshold, uint32_t signal_threshold, uint32_t bits, uint32_t noisy_bits, uint32_t uncertain_bits)
{
    ...
    snprintf(filename, sizeof(filename), "modeac_%04X_%04d.png", modeac, ++file_counter);
    gg::eprint("writing %s\n", filename);
