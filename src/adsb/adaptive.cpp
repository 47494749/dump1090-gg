...
#define ADAPTIVE_DIAG(...) gg::eprint(__VA_ARGS__)
#define ADAPTIVE_DIAG_STDERR(...) fprintf(stderr, __VA_ARGS__)
#else
#define ADAPTIVE_DIAG(...) do {} while (0)
#define ADAPTIVE_DIAG_STDERR(...) do {} while (0)
#endif
...
static bool adaptive_set_gain(int32_t step, const char *why)
{
    ...
    ADAPTIVE_DIAG_STDERR("adaptive: changing gain from %.1fdB (step %d) to %.1fdB (step %d) because: %s\n",
...
void adaptive_init()
{
    ...
    if (maxgain < 0) {
        if (Modes.adaptive_burst_control || Modes.adaptive_range_control) {
            gg::eprint("warning: adaptive gain control requested, but SDR gain control not available, ignored.\n");
        }
    ...
    ADAPTIVE_DIAG("adaptive: using %.0f%% duty cycle\n", 100.0 * N / adaptive_subblock_dutycycle_D);
    ...
    adaptive_range_radix = static_cast<decltype(adaptive_range_radix)>(calloc(65536, sizeof(uint32_t)));
    ...
    ADAPTIVE_DIAG_STDERR("adaptive: enabled adaptive gain control with gain limits %.1fdB (step %d) .. %.1fdB (step %d)\n",
    ...
    if (Modes.adaptive_range_control)
        ADAPTIVE_DIAG("adaptive: enabled dynamic range control, target dynamic range %.1fdB\n", Modes.adaptive_range_target);
    if (Modes.adaptive_burst_control)
        ADAPTIVE_DIAG("adaptive: enabled burst control\n");
    ...
    if (Modes.adaptive_range_control && !adaptive_range_change_timer) {
        ...
        case RANGE_RESCAN_UP:
            if (available_range < Modes.adaptive_range_target) {
                ...
                ADAPTIVE_DIAG("adaptive: available dynamic range (%.1fdB) < required dynamic range (%.1fdB), switching to downward scan\n", available_range, Modes.adaptive_range_target);
                gain_down = gain_not_up = true;
            ...
            if (sdrGetGain() >= adaptive_gain_max) {
                ...
                ADAPTIVE_DIAG("adaptive: reached upper gain limit, halting dynamic range scan here\n");
                adaptive_range_state = RANGE_SCAN_IDLE;
            ...
            if (!gain_not_up) {
                ADAPTIVE_DIAG("adaptive: available dynamic range (%.1fdB) >= required dynamic range (%.1fdB), continuing upward scan\n", available_range, Modes.adaptive_range_target);
                gain_up = true;
        ...
        case RANGE_RESCAN_DOWN:
            if (available_range >= Modes.adaptive_range_target) {
                ...
                ADAPTIVE_DIAG("adaptive: available dynamic range (%.1fdB) >= required dynamic range (%.1fdB), stopping downwards scan here\n", available_range, Modes.adaptive_range_target);
                adaptive_range_state = RANGE_SCAN_IDLE;
            ...
            if (sdrGetGain() <= adaptive_gain_min) {
                ADAPTIVE_DIAG("adaptive: reached lower gain limit, halting dynamic range scan here\n");
                adaptive_range_state = RANGE_SCAN_IDLE;
                adaptive_range_rescan_timer = Modes.adaptive_range_rescan_delay;
                break;
            }
            ...
            ADAPTIVE_DIAG("adaptive: available dynamic range (%.1fdB) < required dynamic range (%.1fdB), continuing downwards scan\n", available_range, Modes.adaptive_range_target);
            gain_down = gain_not_up = true;
            gain_down_reason = "probing dynamic range gain lower bound";
            break;
        ...
        case RANGE_SCAN_IDLE:
            ...
            if (available_range + adaptive_gain_down_db / 2 < Modes.adaptive_range_target && sdrGetGain() > adaptive_gain_min) {
                ADAPTIVE_DIAG_STDERR("adaptive: available dynamic range (%.1fdB) + half gain step down (%.1fdB) < required dynamic range (%.1fdB), starting downward scan\n",
            ...
            if (!adaptive_range_rescan_timer && !gain_not_up) {
                if (available_range >= Modes.adaptive_range_target && sdrGetGain() < adaptive_gain_max) {
                    ADAPTIVE_DIAG("adaptive: start periodic scan for acceptable dynamic range at increased gain\n");
                    gain_up = true;
        ...
        default:
            ADAPTIVE_DIAG("adaptive: in a weird state (%u), trying to fix it\n", (uint32_t) adaptive_range_state);
            adaptive_range_state = RANGE_SCAN_IDLE;
            adaptive_range_rescan_timer = Modes.adaptive_range_rescan_delay;
            break;
        }
