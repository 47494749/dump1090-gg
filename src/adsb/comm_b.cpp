...
static int32_t decodeBDS30(struct modesMessage *mm, bool store)
{
    ...
    if (store) {
        ...
        // Decode BDS 3,0 ACAS RA fields
        mm->acas_ra_valid = 1;
        mm->acas_ara = getbits(msg, 9, 22);       // Active Resolution Advisories (14 bits)
        mm->acas_rac = getbits(msg, 23, 26);      // RA Complement (4 bits)
        mm->acas_rat = getbit(msg, 27);            // RA Terminated
        mm->acas_mte = getbit(msg, 28);            // Multiple Threat Encounter
        mm->acas_tti = getbits(msg, 29, 30);       // Threat Type Indicator (2 bits)
        mm->acas_threat_id = getbits(msg, 31, 56); // Threat Identity Data (26 bits)
    }
...
static int32_t decodeBDS40(struct modesMessage *mm, bool store)
{
    ...
    if (store) {
        ...
        if (mode_valid) {
            ...
            mm->nav.modes = static_cast<nav_modes_t>(
...
static int32_t decodeBDS45(struct modesMessage *mm, bool store)
{
    ...
    // BDS 4,5 Meteorological Hazard Report (MHAR)
    // Bit layout per ICAO Doc 9871 / mode-s.org:
    //   1     : turbulence status
    //   2-3   : turbulence severity (2 bits)
    //   4     : wind shear status
    //   5-6   : wind shear severity (2 bits)
    //   7     : microburst status
    //   8-9   : microburst severity (2 bits)
    //   10    : icing status
    //   11-12 : icing severity (2 bits)
    //   13    : wake vortex status
    //   14-15 : wake vortex severity (2 bits)
    //   16    : temperature status
    //   17    : temperature sign
    //   18-26 : SAT (9 bits, LSB 0.25 C)
    //   27    : pressure status
    //   28-38 : avg static pressure (11 bits, LSB 1 hPa)
    //   39    : radio height status
    //   40-51 : radio height (12 bits, LSB 16 ft)
    //   52-56 : reserved (5 bits, must be zero)
    ...
    uint32_t rh_raw = getbits(msg, 40, 51);   // radio height, 12 bits

    // Bits 52-56 are reserved (5 bits), must be zero
    uint32_t reserved = getbits(msg, 52, 56);

    // Temperature must be valid (per protocol, always present when message is real)
    if (!sat_valid)
        return 0;
    ...
    // Invalid field checks: if status=0, raw bits should be 0
    if (!turbulence_valid && turbulence_raw != 0) return 0;
    if (!windshear_valid && windshear_raw != 0) return 0;
    if (!microburst_valid && microburst_raw != 0) return 0;
    if (!icing_valid && icing_raw != 0) return 0;
    if (!wake_valid && wake_raw != 0) return 0;
    ...
    int32_t radio_height_ft = 0;
    if (rh_valid) {
        radio_height_ft = rh_raw * 16;  // LSB = 16 ft, range 0-65520 ft
        if (radio_height_ft > 65520)
            return 0;
        score += 12;
    } else {
        score += 1;
    ...
    // Count valid hazard fields and apply scoring
    ...
    if (hazard_count >= 2) {
        int32_t severe_count = 0;
        if (turbulence_valid && turbulence_raw == 3) severe_count++;
        if (windshear_valid && windshear_raw == 3) severe_count++;
        if (microburst_valid && microburst_raw == 3) severe_count++;
        if (icing_valid && icing_raw == 3) severe_count++;
        if (wake_valid && wake_raw == 3) severe_count++;
        if (severe_count == hazard_count)
            score -= (hazard_count * 2);  // reduce score for suspicious all-severe
    }

    if (store) {
        mm->commb_format = COMMB_MHAR;

        if (turbulence_valid) {
            mm->mhar_turbulence_valid = 1;
            mm->mhar_turbulence = (hazard_t) turbulence_raw;
        }
        if (windshear_valid) {
            mm->mhar_windshear_valid = 1;
            mm->mhar_windshear = (hazard_t) windshear_raw;
        }
        if (microburst_valid) {
            mm->mhar_microburst_valid = 1;
            mm->mhar_microburst = (hazard_t) microburst_raw;
        }
        if (icing_valid) {
            mm->mhar_icing_valid = 1;
            mm->mhar_icing = (hazard_t) icing_raw;
        }
        if (wake_valid) {
            mm->mhar_wake_valid = 1;
            mm->mhar_wake = (hazard_t) wake_raw;
        }
        if (sat_valid) {
            mm->mhar_sat_valid = 1;
            mm->mhar_sat = sat;
        ...
        if (rh_valid) {
            mm->mhar_rh_valid = 1;
            mm->mhar_rh = (float) radio_height_ft;
        }
    }
...
static int32_t decodeBDS41(struct modesMessage *mm, bool store)
{
    ...
    // Decode 8 characters starting at bit 2
    char waypoint[9];
    waypoint[0] = ais_charset[getbits(msg, 2, 7)];
    waypoint[1] = ais_charset[getbits(msg, 8, 13)];
    waypoint[2] = ais_charset[getbits(msg, 14, 19)];
    waypoint[3] = ais_charset[getbits(msg, 20, 25)];
    waypoint[4] = ais_charset[getbits(msg, 26, 31)];
    waypoint[5] = ais_charset[getbits(msg, 32, 37)];
    waypoint[6] = ais_charset[getbits(msg, 38, 43)];
    waypoint[7] = ais_charset[getbits(msg, 44, 49)];
    waypoint[8] = 0;
    ...
    if (store) {
        ...
        mm->waypoint_valid = 1;
        memcpy(mm->waypoint_id, waypoint, 9);
    }
...
static int32_t decodeBDS42(struct modesMessage *mm, bool store)
{
    ...
    if (store) {
        mm->commb_format = COMMB_WAYPOINT_POS;
        mm->waypoint_lat_valid = 1;
        mm->waypoint_lat = lat;
        mm->waypoint_lon_valid = 1;
        mm->waypoint_lon = lon;
        if (alt_valid) {
            mm->waypoint_alt_valid = 1;
            mm->waypoint_alt = alt;
        }
    }
...
static int32_t decodeBDS43(struct modesMessage *mm, bool store)
{
    ...
    if (store) {
        mm->commb_format = COMMB_WAYPOINT_INFO;
        if (alt_valid) {
            mm->waypoint_crossing_alt_valid = 1;
            mm->waypoint_crossing_alt = alt;
        }
        if (speed_valid) {
            mm->waypoint_crossing_speed_valid = 1;
            mm->waypoint_crossing_speed = speed;
        }
    }
