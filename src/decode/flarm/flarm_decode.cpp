...
void flarm_load_keys_builtin(void)
{
    ...
    flarm_keys_loaded = 1;
    gg::eprint("FLARM: using built-in decryption keys (SoftRF/OGN public)\n");
}
...
bool flarm_load_keys(const char *path)
{
    FILE *f = fopen(path, "r");
    if (!f) {
        gg::eprint("FLARM: cannot open keys file: %s\n", path);
        return false;
    }
    ...
    if (got_table && got_k2 && got_k3 && got_k4 && got_k5) {
        flarm_keys_loaded = 1;
        gg::eprint("FLARM: decryption keys loaded from %s\n", path);
        return true;
    }

    gg::eprint("FLARM: incomplete keys file %s (need key_table, key2, key3, key4, key5)\n", path);
    return false;
}
