    ...
    if (corrections < 0) {
        ...
        if (violations <= 2 && (type_field == 3 || type_field == 4)) {
            ...
            if (type_field == 3) {
                ...
                gg::eprint("flarm: type=3 addr=%06X (undocumented protocol, not decodable)\n", addr);
            } else {
...
{
    struct flarm_state *s = static_cast<struct flarm_state*>(calloc(1, sizeof(struct flarm_state)));
    if (!s) return NULL;
