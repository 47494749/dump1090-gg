...
void flarmReaderInitConfig(void)
{
    ...
    FlarmConfig.ogn_server = "aprs.glidernet.org";
...
static void *flarm_ifile_reader_thread(void *arg)
{
    ...
    const char *path = FlarmConfig.ifile_path.c_str();
    const uint32_t buf_size = MODES_RTL_BUF_SIZE; // 256 KB
    uint8_t *buf = static_cast<uint8_t*>(malloc(buf_size));
    if (!buf) {
        gg::eprint("flarm-ifile: failed to allocate read buffer\n");
        return NULL;
    }

    gg::eprint("flarm-ifile: reader thread started, file=%s\n", path);
    ...
    while (!Flarm.stop_flag && !Modes.exit) {
        ...
        if (!fp) {
            gg::eprint("flarm-ifile: cannot open '%s': %s\n", path, strerror(errno));
            break;
        }

        if (loop_count > 0) {
            gg::eprint("flarm-ifile: replaying file (loop %u)\n", loop_count);
        }
        loop_count++;
        ...
        while (!Flarm.stop_flag && !Modes.exit) {
            ...
            if (nread == 0) {
                if (FlarmConfig.ifile_once) {
                    gg::eprint("flarm-ifile: reached EOF after one pass, requesting shutdown\n");
                    Modes.exit = 1;
                }
                break;
            }
    ...
    free(buf);
    gg::eprint("flarm-ifile: reader thread exiting (loops=%u)\n", loop_count);
    return NULL;
}
...
static void *flarm_reader_thread(void *arg)
{
    ...
    gg::eprint("flarm: reader thread started, reading async...\n");
    rtlsdr_read_async(Flarm.dev, flarm_rtlsdr_callback, NULL, 4, MODES_RTL_BUF_SIZE);

    if (!Modes.exit && !Flarm.stop_flag) {
        gg::eprint("flarm: rtlsdr_read_async returned unexpectedly\n");
    }
#endif

    gg::eprint("flarm: reader thread exiting\n");
    return NULL;
}
...
bool flarmReaderOpen(void)
{
    ...
    if (!FlarmConfig.ifile_path.empty()) {
        ...
        FILE *fp = fopen(FlarmConfig.ifile_path.c_str(), "rb");
        if (!fp) {
            fprintf(stderr, "flarm-ifile: cannot open '%s': %s\n",
                    FlarmConfig.ifile_path.c_str(), strerror(errno));
            return false;
        }
        ...
        fprintf(stderr, "flarm-ifile: file '%s' (%ld bytes, %.1f seconds at %.1f MSPS)\n",
                FlarmConfig.ifile_path.c_str(), file_size, duration, sample_rate / 1e6);
        ...
        if (!Flarm.demod) {
            gg::eprint("flarm-ifile: failed to create demodulator\n");
            return false;
        }

        // Use file mtime as timestamp for XXTEA decryption
        struct stat st;
        if (stat(FlarmConfig.ifile_path.c_str(), &st) == 0) {
            uint32_t file_time = (uint32_t)st.st_mtime;
            flarm_demod_set_time_override(Flarm.demod, file_time);
            gg::eprint("flarm-ifile: using file mtime %u for decryption\n", file_time);
        }
        ...
        if (FlarmConfig.p3i_enabled) {
            ...
            if (Flarm.p3i_demod) {
                gg::eprint("flarm-ifile: P3I decoder enabled (869.525 MHz)\n");
            }
        }
...
#ifdef ENABLE_RTLSDR
    if (FlarmConfig.device_serial.empty()) {
        gg::eprint("flarm: no device serial specified (use --flarm-device or --flarm-ifile)\n");
        return false;
    }

    int32_t dev_index = find_flarm_device(FlarmConfig.device_serial.c_str());
    if (dev_index < 0) {
        gg::eprint("flarm: no RTL-SDR device with serial '%s' found\n", FlarmConfig.device_serial.c_str());

        // List available devices
        int32_t count = rtlsdr_get_device_count();
        gg::eprint("flarm: available devices:\n");
        for (int32_t i = 0; i < count; i++) {
            char vendor[256], product[256], sn[256];
            if (rtlsdr_get_device_usb_strings(i, vendor, product, sn) == 0) {
                gg::eprint("  %d: %s %s SN:%s\n", i, vendor, product, sn);
            }
        }
        return false;
    }
    ...
    fprintf(stderr, "flarm: opening RTL-SDR device #%d (SN: %s) for 868 MHz\n",
            dev_index, FlarmConfig.device_serial.c_str());

    if (rtlsdr_open(&Flarm.dev, dev_index) < 0) {
        gg::eprint("flarm: error opening RTL-SDR device: %s\n", strerror(errno));
        return false;
    }
    ...
    Flarm.demod = flarm_demod_create(&demod_config);
    if (!Flarm.demod) {
        gg::eprint("flarm: failed to create demodulator\n");
    ...
    if (FlarmConfig.p3i_enabled) {
        ...
        Flarm.p3i_demod = p3i_demod_create(&p3i_cfg);
        if (Flarm.p3i_demod) {
            gg::eprint("flarm: P3I decoder enabled (869.525 MHz)\n");
        }
    }
...
#else
    gg::eprint("flarm: RTL-SDR support not compiled in\n");
    return false;
#endif
}
...
void flarmReaderStart(void)
{
    ...
    if (!FlarmConfig.ifile_path.empty()) {
        ...
        gg::eprint("flarm-ifile: reader thread started\n");
        return;
    }
    ...
    gg::eprint("flarm: reader thread started\n");
#endif
}
...
void flarmReaderStop(void)
{
    ...
    gg::eprint("flarm: reader thread stopped\n");
}
...
{
    flarm_decoder_state_t *st = static_cast<flarm_decoder_state_t*>(calloc(1, sizeof(*st)));
    if (!st) return false;
    ...
    if (FlarmConfig.p3i_enabled) {
        ...
        if (st->p3i_queue) {
            ...
            if (st->p3i_demod) {
                gg::eprint("rx[%d]: P3I decoder enabled (869.525 MHz)\n", rx->id);
            }
        }
    }

    // Initialize OGN client if station is configured
    if (!FlarmConfig.ogn_station.empty()) {
        ognClientInit();
    }
    ...
    gg::eprint("rx[%d]: FLARM decoder created (868 MHz GFSK)\n", rx->id);
    return true;
}
...
void flarmDecoderStop(struct sdr_receiver *rx)
{
    ...
    gg::eprint("rx[%d]: FLARM decoder destroyed\n", rx->id);
}
...
bool flarmReaderHandleOption(int argc, char **argv, int *jptr)
{
    ...
    if (!strcmp(argv[j], "--flarm")) {
        ...
        FlarmConfig.enabled = 1;
        FlarmConfig.device_serial = argv[++j];
    } else if (!strcmp(argv[j], "--flarm-ifile") && more) {
        FlarmConfig.enabled = 1;
        FlarmConfig.ifile_path = argv[++j];
        ...
        FlarmConfig.keys_file = argv[++j];
    } else if (!strcmp(argv[j], "--ogn-station") && more) {
        FlarmConfig.ogn_station = argv[++j];
    } else if (!strcmp(argv[j], "--ogn-server") && more) {
        FlarmConfig.ogn_server = argv[++j];
