...
static int32_t viterbi_decode(const int8_t *soft_bits, int32_t coded_len, uint8_t *out_bits)
{
    ...
    uint64_t *path = static_cast<uint64_t*>(calloc((size_t)n_out, sizeof(uint64_t) * VITERBI_STATES));
    if (!path) return 0;
...
{
    struct lte_state *st = static_cast<struct lte_state*>(calloc(1, sizeof(struct lte_state)));
    if (!st) return NULL;
    ...
    st->iq_buf = static_cast<decltype(st->iq_buf)>(calloc((size_t)st->iq_buf_size * 2, sizeof(float)));
                       ...
                       int32_t pci, float freq_offset_hz, lte_mib_t *mib)
{
    ...
    if (++mib_dbg_cnt % 20 == 1) {
        ...
        gg::eprint("LTE MIB: H_phase[mid]=%.2f,%.2f |H|=%.1f\n", ph0, ph1, mag);
    }
                                ...
                                if (n_rb <= 6 && mib.sfn % 8 == 0) {
                                    ...
                                    if (lte_decode_pdcch_si(st->iq_buf, st->iq_buf_len,
                                        ...
                                        if (lte_decode_pdsch_sib(st->iq_buf, st->iq_buf_len,
                                            ...
                                            lte_sib1_info_t sib1_full;
                                            if (lte_parse_sib1(tb_bits, tb_len, &sib1_full)) {
                                                cell->info.sib1.mcc = sib1_full.plmn[0].mcc;
                                                cell->info.sib1.mnc = sib1_full.plmn[0].mnc;
                                                cell->info.sib1.tac = sib1_full.tac;
                                                cell->info.sib1.cell_id = sib1_full.cell_id;
                                                cell->info.sib1.cell_barred = sib1_full.cell_barred;
                                                cell->info.sib1.q_rxlevmin = sib1_full.q_rxlevmin;
                                                cell->info.sib1.si_window_length = sib1_full.si_window_length;
                                                cell->info.sib1.valid = true;
                                                cell->info.sync_state = LTE_SYNC_SIB1;
                                                cell->info.sib1_count++;
                                                st->stats.sib1_decoded++;
                                                fprintf(stderr, "LTE SIB1: PCI=%d MCC=%u MNC=%u TAC=%u CellID=0x%07X\n",
                                                        pci, sib1_full.plmn[0].mcc, sib1_full.plmn[0].mnc,
                                                        sib1_full.tac, sib1_full.cell_id);
                                            }
                                        }
                                    }
                                    ...
                                    for (int32_t si = 0; si < 5; si++) {
                                        ...
                                        // Parse SI message (may contain SIB10-14 alerts)
                                        lte_sib_results_t si_res;
                                        si_res = {};
                                        ...
                                        if (si_res.sib10.valid && cell->info.alert_count < LTE_MAX_ALERTS) {
                                            ...
                                            a->type = LTE_ALERT_ETWS;
                                            a->message_id = si_res.sib10.message_id;
                                            a->serial_number = si_res.sib10.serial_number;
                                            a->timestamp = (uint64_t)time(NULL);
                                            a->active = true;
                                            snprintf(a->category, sizeof(a->category), "%s",
                                                     lte_etws_warning_type(si_res.sib10.warning_type[0]));
                                            a->text[0] = '\0'; // Primary has no text
                                        }
                                        if (si_res.sib11.valid && cell->info.alert_count < LTE_MAX_ALERTS) {
                                            lte_alert_t *a = &cell->info.alerts[cell->info.alert_count++];
                                            a->type = LTE_ALERT_ETWS;
                                            a->message_id = si_res.sib11.message_id;
                                            a->serial_number = si_res.sib11.serial_number;
                                            a->timestamp = (uint64_t)time(NULL);
                                            a->active = true;
                                            snprintf(a->category, sizeof(a->category), "ETWS Secondary");
                                            lte_cbs_decode_text(si_res.sib11.warning_msg,
                                                si_res.sib11.warning_msg_len,
                                                si_res.sib11.data_coding_scheme,
                                                a->text, sizeof(a->text));
                                        }

                                        // Check for CMAS (SIB12)
                                        if (si_res.sib12.valid && cell->info.alert_count < LTE_MAX_ALERTS) {
                                            lte_alert_t *a = &cell->info.alerts[cell->info.alert_count++];
                                            a->type = LTE_ALERT_CMAS;
                                            a->message_id = si_res.sib12.message_id;
                                            a->serial_number = si_res.sib12.serial_number;
                                            a->timestamp = (uint64_t)time(NULL);
                                            a->active = true;
                                            snprintf(a->category, sizeof(a->category), "%s",
                                                     lte_cmas_category(si_res.sib12.message_id));
                                            lte_cbs_decode_text(si_res.sib12.warning_msg,
                                                si_res.sib12.warning_msg_len,
                                                si_res.sib12.data_coding_scheme,
                                                a->text, sizeof(a->text));
                                        }

                                        // Check for EAB (SIB14)
                                        if (si_res.sib14.valid && cell->info.alert_count < LTE_MAX_ALERTS) {
                                            lte_alert_t *a = &cell->info.alerts[cell->info.alert_count++];
                                            a->type = LTE_ALERT_EAB;
                                            a->message_id = 0;
                                            a->serial_number = 0;
                                            a->timestamp = (uint64_t)time(NULL);
                                            a->active = true;
                                            ...
                                            for (int32_t ac = 0; ac < 10; ac++) {
                                                if (si_res.sib14.ac_barring[ac].barred) {
                                                    tpos += snprintf(a->text + tpos,
                                                        ...
                                                        "AC%d=%c ", ac,
                                                        'a' + si_res.sib14.ac_barring[ac].category);
                                                }
                                            }
                                        }
                                    }
