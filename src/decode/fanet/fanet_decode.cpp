...
#define FANET_DIAG(...) fprintf(stderr, __VA_ARGS__)
#else
#define FANET_DIAG(...) do {} while (0)
#endif
...
static void decode_packet(fanet_state_t *s)
{
...
#ifdef FANET_DEBUG
    gg::eprint("DBG: hdr_symbols (masked to %d bits): ", sf_hdr);
    for (int32_t i = 0; i < header_cw_len; i++) gg::eprint("%d ", hdr_symbols[i]);
    gg::eprint("\n");
#endif
...
#ifdef FANET_DEBUG
    gg::eprint("DBG: hdr_codewords (%d): ", hdr_num_cw);
    for (int32_t i = 0; i < hdr_num_cw; i++) gg::eprint("0x%02X ", hdr_codewords[i]);
    gg::eprint("\n");
#endif
...
#ifdef FANET_DEBUG
    gg::eprint("DBG: hdr_nibbles (%d): ", hdr_num);
    for (int32_t i = 0; i < hdr_num; i++) gg::eprint("0x%X ", hdr_nibbles[i]);
    gg::eprint("\n");
#endif
...
#ifdef FANET_DEBUG
    gg::eprint("DBG: expected_len=%d, payload_cr=%d, has_crc=%d\n", expected_len, payload_cr, has_crc);
#endif
    ...
    if (has_crc) {
        ...
        if (crc_recv == crc_a || crc_recv == crc_b) {
            ...
            s->stats.crc_errors++;
                FANET_DIAG("fanet: CRC fail sym=%d len=%d cr=%d recv=%04X calcA=%04X calcB=%04X type=%u src=%02X:%02X%02X\n",
    ...
    if (parse_fanet_packet(payload, payload_len, &msg)) {
        FANET_DIAG("FANET: %s type=%u src=%02X:%04X len=%d preamble_off=%d\n",
...
{
    fanet_state_t *s = static_cast<fanet_state_t*>(calloc(1, sizeof(fanet_state_t)));
    if (!s) return NULL;

    s->sample_rate = sample_rate;
    s->samples_per_symbol = (uint32_t)((double)sample_rate * LORA_N / FANET_BW);

    s->dechirp_i = static_cast<decltype(s->dechirp_i)>(calloc(s->samples_per_symbol, sizeof(float)));
    s->dechirp_q = static_cast<decltype(s->dechirp_q)>(calloc(s->samples_per_symbol, sizeof(float)));
    s->downchirp_i = static_cast<decltype(s->downchirp_i)>(calloc(s->samples_per_symbol, sizeof(float)));
    s->downchirp_q = static_cast<decltype(s->downchirp_q)>(calloc(s->samples_per_symbol, sizeof(float)));
    s->sym_buf_i = static_cast<decltype(s->sym_buf_i)>(calloc(s->samples_per_symbol, sizeof(float)));
    s->sym_buf_q = static_cast<decltype(s->sym_buf_q)>(calloc(s->samples_per_symbol, sizeof(float)));
    s->fft_mag = static_cast<decltype(s->fft_mag)>(calloc(LORA_N, sizeof(float)));
    ...
    FANET_DIAG("fanet: decoder created, sr=%u, sps=%u, sym_rate=%.1f Hz, sync=0x%02X\n",
