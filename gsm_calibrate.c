...
#define SAMPLES_PER_SCAN (16384 * 64)  // 1048576 IQ pairs = 2097152 bytes (aligned to USB bulk), ~0.52 sec
...
gsm_cal_result_t gsm_calibrate(const char *serial, int current_ppm, float gain_db)
{
    ...
    int dev_index = rtlsdr_get_index_by_serial(serial);
    if (dev_index < 0) {
        snprintf(result.error, sizeof(result.error),
                 "RTL-SDR with serial %s not found", serial);
        return result;
    }
    ...
    if (rtlsdr_open(&dev, (uint32_t)dev_index) < 0) {
        snprintf(result.error, sizeof(result.error),
                 "Failed to open RTL-SDR device #%d", dev_index);
        return result;
    }

    // Configure SDR
    rtlsdr_set_sample_rate(dev, SAMPLE_RATE);
    rtlsdr_set_freq_correction(dev, current_ppm);

    // Always use auto-gain for GSM scanning (BTS signals are strong)
    (void)gain_db;
    rtlsdr_set_tuner_gain_mode(dev, 0);

    // Allocate buffers
    iq_buf     = malloc(SAMPLES_PER_SCAN * 2);
    ...
    for (int scan = 0; scan < scans && ppm_count < MAX_PPM_VALUES; scan++, freq += freq_step) {
        ...
        int actual_freq = (int)rtlsdr_get_center_freq(dev);
        rtlsdr_reset_buffer(dev);

        // Read IQ samples
        int n_read = 0;
        int rc = rtlsdr_read_sync(dev, iq_buf, SAMPLES_PER_SCAN * 2, &n_read);
        // Accept data even with LIBUSB_ERROR_OVERFLOW (-8) — common when buffer
        // isn't perfectly aligned; the data is still usable if we got enough.
        ...
        // Compute averaged power spectrum using sliding FFT
        ...
        for (int off = 0; off + FFT_SIZE <= num_iq_pairs; off += slide_step) {
            // Convert uint8 IQ to windowed complex float
            ...
            // Accumulate power with fftshift (DC to center)
            for (int i = 0; i < FFT_SIZE; i++) {
...
gsm_cal_result_t gsm_calibrate(const char *serial, int current_ppm, float gain_db)
{
    (void)serial; (void)current_ppm; (void)gain_db;
    gsm_cal_result_t result;
    memset(&result, 0, sizeof(result));
    snprintf(result.error, sizeof(result.error), "RTLSDR support not compiled in");
    return result;
}
