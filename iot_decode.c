...
// Honeywell CM921/CM927: FSK, ~38.4 kbps, Manchester encoded
static bool decode_honeywell_cm(const uint8_t *bits, int bit_count, iot_device_msg_t *msg)
{
    ...
    for (int i = 0; i < bit_count - 120; i += 8) {
        // Look for transition from all-1s to 0x00 0x33
    ...
    int byte_count = 0;
    int bit_idx = 0;
    for (int i = start; i + 1 < bit_count && byte_count < 16; i += 2) {
    ...
    // Honeywell frame: device_id(3), command(1), payload_len(1), payload...
    uint32_t dev_id = (bytes[0] << 16) | (bytes[1] << 8) | bytes[2];

    msg->protocol = IOT_PROTO_HONEYWELL_CM;
    msg->modulation = IOT_MOD_FSK;
    msg->device_id = dev_id;
    msg->channel = bytes[3];  // command byte as channel
    msg->temperature_c = NAN;
    msg->humidity_pct = NAN;
    msg->pressure_hpa = NAN;
    msg->wind_speed_ms = NAN;
    ...
    // If command is temperature report (0x30), try to extract
    if (bytes[3] == 0x30 && byte_count >= 8) {
